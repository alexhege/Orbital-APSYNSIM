...
class Interferometer(object):
  ...
  #  else:
    ...
    self.gamma = 1.0 #0.5  # Gamma correction to plot model. gamma=.5 means brighten the less bright parts
    #Hegedus this messes up uv plot, too far zoomed out
    #self.lfac = 1.e6   # Lambda units (i.e., 1.e6 => Mlambda)
    #self.ulab = r'U (M$\lambda$)'
    #self.vlab = r'V (M$\lambda$)'
    self.lfac = 1.e3   # Lambda units (i.e., 1.e6 => Mlambda)
...
# Default of defaults!
    ...
    DefaultMod = 'CME_west.model' #'Nebula.model'
    DefaultArray = 'CME-array_0000sec.array' #'Sunrise.array' #'Golay_12.array'
...
#   execfile(os.path.join(os.path.basename(d1),'APSYNSIM.config'))
    try:
      conf = open(os.path.join(d1,'relicAPSYNSIM.config'))
    except:
      d1 = os.getcwd()
      conf = open(os.path.join(d1,'relicAPSYNSIM.config'))
...
# Set instance configuration values:
 ...
 # Try to read a default initial model:
   ...
   #Took a bunch of stuff here and inserted it into beginning of gui to make looping for gifs easier
  ...
  def GUI(self): # ,makefigs=True):
    ...
    #Hegedus this part is the globe in the uvplane, showing the track of the earth's rotation
    ...
    #Hegedus wax is starting pos from lower left it looks like, and then size is 3rd and 4th args 
    ...
    if not gifMode:
      ...
      self._plotModel()
      self._plotModelFFT()
      self._plotDirty(stepSize = stepSize)
      #print 'robfac is ' +  str(self.robfac)
      #print 'tot is ' +  str(np.sum(self.totsampling))
    ...
    if gifMode:
      ...
      for i in range(0, self.nH - stepSize+1, stepSize):
        ...
        target.write('DIAMETER  ' + str(self.Diameters[0]) + ' \n')

        for j in range(self.Nant):
          target.write('ANTENNA  ' + str(self.projpos[j][i+stepSize-1][0]) + '  ' + str(self.projpos[j][i+stepSize-1][1]) + '\n')
        ...
        self._plotAntennas(redo=True,rescale=True, gifStep = i+stepSize)
        

        self._plotDirty(gifStep = i, stepSize = stepSize)
        #print 'robfac is ' +  str(self.robfac)
        #print 'tot is ' +  str(np.sum(self.totsampling))
    ...
    #quit Hegedus switch this on/off to change if it's interactive or just saves figure and quits immediately
    autoQuit = True
    if autoQuit:
      self.quit()
      ...
      for li,l in enumerate(fi.readlines()):
        ...
        if len(it)>0:
          if it[0]=='ANTENNA':   
            ...
            Nant += 1
            #Hegedus comment this line out, bc we are already in km
  ...
  def readModels(self,model_file):
    ...
    if len(model_file)>0: 
     if not os.path.exists(model_file):
      ...
      ###Hegedus added this to get to preferred wl automatically
      #self.wavelength*=.1 #goes to .006 km
      #self.wavelength = .010 ### wavelength in m, sed file changes this line to go over many wl's, triple '#' needed
      ##
  ...
  def _changeCoordinates(self,rescale=False,redoUV=False):
    ...
    newtext = self.fmtH%(self.nH)#(self.lat/self.deg2rad,self.dec/self.deg2rad,self.Hcov[0]/self.Hfac,self.Hcov[1]/self.Hfac)
  ...
  def _prepareBaselines(self):
    ...
    if self.nH < 3:
      self.B = np.zeros((NBmax,3),dtype=np.float32)
    ...
    #Can add file determining initial gains here
    ...
    if gainNoise:
    ##Hegedus now update gains to be randomly nonideal, simulate noise or bad pointing
      self.Gains = self.Gains + np.random.normal(0, gainNoiseRMS[0], np.shape(self.Gains))
      self.Gains = self.Gains + np.random.normal(0, gainNoiseRMS[1], np.shape(self.Gains))*1.j
    ...
    if posUncertain:
      for i in range(self.Nbas):
        for j in range(self.nH):
          dtau = np.random.normal(0, posUncert*np.sqrt(2./3.)) + np.random.normal(0, clockUncert*np.sqrt(2.))
          uAngle = dtau*3e8/(self.wavelength*1000.)*2*np.pi
          self.Gains[i][j] *= np.cos(uAngle) + np.sin(uAngle)*1.j #multiply by this to rotate gain


    ###Hegedus This whole part doesn't matter to me, except for antnum, workaround that
    bi = 0
    ...
    ###Hegedus Nant2 is always 0 for us, only used when combining arrays
  ...
  def _setBaselines(self,antidx=-1):
   ...
   #Hegedus 
   ###This is the key!!!
    # trlat is (sin, cos) of lat, H is sin cos of all intermediate hour vals
    ### is declination here just the elevation/altitude angle?  Yes

   #self.trlat = (0, 1)
   #self.H = (0, 1) #update this next,
   #Right now H goes from 0 to .1, so it's mostly (0, 1) 


   for currBas in bas2change:
    for h in range(self.nH):
     self.u[currBas, h] = self.baselines[currBas][h][0]/self.wavelength
     self.v[currBas, h] = self.baselines[currBas][h][1]/self.wavelength
  ...
  def _gridUV(self,antidx=-1, gifStep = -1):
   ...
   for nb in bas2change:
     pixU = np.rint(self.u[nb, :gifStep]/self.UVpixsize).flatten().astype(np.int32)
     pixV = np.rint(self.v[nb, :gifStep]/self.UVpixsize).flatten().astype(np.int32)
     ...
     #insert hegedus alt version here
     self.pixpos[nb] = [np.copy(pU),np.copy(pV),np.copy(mU),np.copy(mV)]
     gpix = np.zeros((self.nH), dtype=bool)
     gpix[:gifStep]= goodpix
     for k in range(len(pV)):
       self.totsampling[pV[k], mU[k]] += 1.0
       self.totsampling[mV[k], pU[k]] += 1.0
       self.Gsampling[pV[k],mU[k]] += self.Gains[nb,gpix][k]
       self.Gsampling[mV[k],pU[k]] += np.conjugate(self.Gains[nb,gpix][k])
    ...
    #   print np.shape(goodpix), np.shape(self.Gains), np.shape(self.pixpos[nb][0]), nb
     ...
     #copy from thermal part in dirty
     #gpix = np.zeros((self.nH), dtype=bool)
     #gpix[:gifStep] = goodpix
     #self.Gsampling[pV,mU] += self.Gains[nb,gpix]
     #self.Gsampling[mV,pU] += np.conjugate(self.Gains[nb,gpix])
     #accumGsampling[pV,mU] += self.Gains[nb,gpix]
     #accumGsampling[mV,pU] += np.conjugate(self.Gains[nb,gpix])




   self.robfac = (5.*10.**(-self.robust))**2.*(2.*self.Nbas*gifStep)/np.sum(self.totsampling**2.)
  ...
  def _setBeam(self,antidx=-1, gifStep=-1):

   self._gridUV(antidx=antidx, gifStep = gifStep) 
 ...
 #  self.beamScale = np.max(self.beam[self.Nphf:self.Nphf+1,self.Nphf:self.Nphf+1])
  ...
  def _prepareModel(self):
    ...
    #print 'INFO ' + str(sh0) + ' '+ str(sh1) + ' '+ str(zd0) + ' '+ str(zd1) + ' '+ str(Np4) + ' '

    ###Hegedus adding gaussian random noise 
    #self.modelimTrue += np.random.normal(0, np.amax(self.modelimTrue)*.01, np.shape(self.modelimTrue))
  ...
  def _plotDirty(self,redo=True, gifStep = -1, stepSize = 1):
    ...
    if gifStep != -1:
      ...
      for nb in range(self.Nbas):
        ...
        #self.accumGsampling[pV,mU] += self.Gains[nb,gpix]
        #self.accumGsampling[mV,pU] += np.conjugate(self.Gains[nb,gpix])

      #print 'in loop tot is so far ' + str(np.sum(totsampling))
      #print 'max in loop tot is so far ' + str(np.amax(totsampling))
      ###Hegedus adding gaussian random noise
      ...
      #print 'max Gsamp ' + str(np.amax(Gsampling)) + ' at ' + str(np.where(Gsampling == np.amax(Gsampling)))    
      self.accum += modelfft*np.fft.ifftshift(Gsampling)
      #print 'max accum ' + str(np.amax(self.accum)) + ' at ' + str(np.where(self.accum == np.amax(self.accum)))
      #self.accum = modelfft*np.fft.ifftshift(self.accumGsampling)
      #print 'max accumGsamp ' + str(np.amax(self.accumGsampling)) + ' at ' + str(np.where(self.accumGsampling == np.amax(self.accumGsampling)))
      #print 'max accum ' + str(np.amax(self.accum)) + ' at ' + str(np.where(self.accum == np.amax(self.accum)))
      
      #accum = self.accum/(1.+self.robfac*self.totsampling)  
      accum = np.fft.ifftshift(np.fft.fftshift(self.accum)/(1.+self.robfac*self.totsampling))
    ...
    #not in gifmode, gifstep == -1  
      ...
      if thermalNoise:
       ...
       #slice it up so these are sampled from current noisy model part 
        ...
        accum = np.fft.ifftshift(np.fft.fftshift(accum)/(1.+self.robfac*self.totsampling))
  ...
  def _plotAntennas(self,redo=True,rescale=False, gifStep = -1):
   ...
   #Hegedus, this messes uv axes up, too far zoomed out
   ...
   if redo:
    ...
    #Hegedus change bounds makes inital plot in good scale

    #largestbl = np.amax(abs(self.baselines)) + 50. #800

    # longest baseline is about 600 km, this keeps the axes on same scale!
    self.antPlot.set_xlim((np.amin(np.array(self.antPos)[:, 0] - 50), np.amin(np.array(self.antPos)[:, 0]) + self.largestbl))
    self.antPlot.set_ylim((np.amin(np.array(self.antPos)[:, 1] - 50), np.amin(np.array(self.antPos)[:, 1]) + self.largestbl))
    ###
    ...
    self.UVPlotPlot = []
    toplotu = self.u[:, :gifStep].flatten()/self.lfac ;  toplotv = self.v[:, :gifStep].flatten()/self.lfac ; 
    ...
    #Hegedus change bounds makes initial plot in good scale
    ...
    self.latText = self.UVPlot.text(0.05,0.87,self.fmtH%(gifStep),transform=self.UVPlot.transAxes)
  ...
  def _plotBeam(self,redo=True):
    ...
    ###Hegedus this is where the path on the sphere 
...
#Hegedus take out all sphere plotting stuff
