...
class Interferometer(object):
  ...
  def quit(self,event=None):
    ...
    saveFig = True
    gifName = False
    baseFigName = '/media/sf_XubuntuShare/RadGal/RadGal_5.png'
    figName = baseFigName
    if gifName:
    ...
    saveRMSE = True
    RMSEFile = '/media/sf_XubuntuShare/RadGal/rmse_5.dat'
    if saveRMSE:
      Np4 = self.Npix/4
 ...
 # Try to read a default initial model:
  ...
  def _prepareBaselines(self):
    ...
    ##Hegedus now update gains to be randomly nonideal, simulate noise or bad pointing
      ...
      #    self.Gains[i][j] = 1*np.random.random() + .0 + 0.j
...
#   for currBas in bas2change:
#    for h in range(self.nH):
#     self.B[currBas, 0] = -self.baselines[currBas][h][1]*self.trlat[0]/self.wavelength
#     self.B[currBas, 1] = self.baselines[currBas][h][0]/self.wavelength
#     self.B[currBas, 2] = self.baselines[currBas][h][1]*self.trlat[1]/self.wavelength
#     self.u[currBas, h] = -(self.B[currBas,0]*self.H[0][0][h] + self.B[currBas,1]*self.H[1][0][h])
#     self.v[currBas, h] = -self.B[currBas,0]*self.trdec[0]*self.H[1][0][h]+self.B[currBas,1]*self.trdec[0]*self.H[0][0][h]+self.trdec[1]*self.B[currBas,2]
 ...
 #  self.beamScale = np.max(self.beam[self.Nphf:self.Nphf+1,self.Nphf:self.Nphf+1])
  ...
  def _plotDirty(self,redo=True):
    ...
    if thermalNoise:
      ...
      Ae = .9*((800000./2.)**2*np.pi)
      k = 1.38e-23
      sefd = 2*k*Tgal/Ae*10**26
      Srms =  sefd/(np.sqrt(.1*32000))
      if customNoise:
        Srms = customRMS
  ...
  def _plotAntennas(self,redo=True,rescale=False):
   ...
   if redo:
    ...
    # longest baseline is about 800 km, this keeps the axes on same scale!
