...
#Change inbase stuff
inbaseFile="${APSYNSIMDir}/SCRIPT/vlbainbase.py"
ranges=$(cat $paramFile | grep 'ranges[[:space:]]*=' | awk -F = '{print $2}'| grep '(*)')
sed -i "s/JDranges =.*)/JDranges = $ranges/" $inbaseFile
...
JDstep=$(cat $paramFile | grep 'stepJD[[:space:]]*=' | awk -F = '{print $2}'|awk '{print $1}')
...
sed -i -r "s/JDstep = [-+]?[0-9]*\.?[0-9]+\.?/JDstep = $JDstep/" $inbaseFile
...
#Gif Mode!!! 
  ...
  tot=$(python vlbahelp.py "$ranges" "$JDstep") 
